...
public class DAO {
	...
	public static void redistribute(int candidateID, int noOfCand){
		...
		try {
			String query="SELECT voterID FROM ballotpaper WHERE candidateID=? AND preference=1"; //find this candidate where their preference is 1
			PreparedStatement pstmt = conn.prepareStatement(query);
			pstmt.setInt (1, candidateID);
		    rs = pstmt.executeQuery();
			while(rs.next()){
				voterIDs.add(rs.getInt("voterID"));//add the voters who voted this candidate as number 1
			}
			for(int i=0; i<voterIDs.size(); i++){ //loop through these voters
				...
				while(rs.next()){
					preferences.add(rs.getInt("preference"));//get each preference and 
					candIDs.add(rs.getInt("candidateID")); //candidateid for this voters i.e. getting their entire ballot card 
				}
			}
			...
			for(int i=0; i<tempPref.size()/noOfCand; i++){
				for(int j=y; j<x-1; j++){
					Collections.swap(tempPref, j+1, j);
					if(j%noOfCand==0){
						while(alreadyElected(tempPref.get(j))){
							Collections.swap(tempPref, j+noOfCand-1, j);
							Collections.swap(tempPref, j+1, j);
						}
					}
				}
	...
	public static boolean alreadyElected(int x){
		ResultSet rs=null;
		try {
			String query = "SELECT * FROM electedCandidate "
					+ "WHERE idelectedcandidate= ? ";
			PreparedStatement pstmt = conn.prepareStatement(query);
		    pstmt.setInt(1, x); 
		    rs = pstmt.executeQuery();
		    if(rs.next()){
	...
	public static void addToElected(int x) {
		try{
			...
			String query = "INSERT INTO electedcandidate (idElectedCandidate)" 
					+ " VALUES (?)";
